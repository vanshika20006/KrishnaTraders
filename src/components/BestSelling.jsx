...
// Assuming this component is in a separate file (ProductCard.jsx)
const ProductCard = ({ image, name, inStock, price, size, slug }) => {
  const navigate = useNavigate();

  const handleClick = () => {
    navigate(`/product${slug}`, {
      state: { name, price, inStock, image, size, slug },
    });
  };

  const getImageUrl = (imgName) => {
    // Fallback to placeholder if image path is not valid
    // This is a common practice for robustness
    const validImagePaths = [
      "/Aqua24X24.jpg",
      "POLOSHORTBODY.jpg",
      "HEAVY24X24.jpg",
      "LDPE24X24.jpg",
      "/Short-body-JERRY.jpg",
      "/Short-body-BREZZA.jpg",
      "/OPELBIBCOCK.jpg",
      "/OneTef.png",
      "/EVERYPTMT.jpg",
      "/DShortbody1.PNG",
      "/FShortbody1.PNG",
      "/FLUSH-COCK-M-BREZZA.jpg",
      "/Angle-PROJECT-1st.jpg",
      "/UPVCF.jpg",
      "/CPVCF.jpg",
      "/cpvc118.jpg"
    ];

    if (validImagePaths.includes(imgName) || imgName.startsWith('data:')) {
      return imgName;
    }
    return `https://placehold.co/200x200?text=${name.substring(0, 10)}`;
  };
  ...
  return (
    ...
    >
      {/* Image Section */}
      <div className="w-full h-36 lg:h-40 flex items-center justify-center p-2 lg:p-4">
        <img
      ...
      {/* Details Section */}
      <div className="bg-white pt-1 text-center">
        <h6 className="font-semibold text-base lg:text-lg">{name}</h6>
        <p className={`text-xs ${inStock ? "text-green-600" : "text-red-600"} lg:text-sm`}>
          {inStock ? "In Stock" : "Out of Stock"}
        </p>
        <p className="text-gray-900 font-bold text-sm mt-1 lg:text-base">Rs. {price}</p>
        <p className="text-gray-600 text-xs mt-0.5 lg:text-sm">Size: {size}</p>
      </div>
    </div>
  );
};
...
const BestSelling = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-[#f9f9e6] flex flex-col lg:flex-row">
      {/* Mobile Header with Title and Buttons */}
      <div className="lg:hidden p-4 flex flex-col gap-4 border-b border-gray-200 sticky top-0 bg-[#f9f9e6] z-10">
        <h2 className="text-xl font-semibold text-[#1a1f2c]">BestSelling Products</h2>
        <div className="flex gap-4">
          ...
          <div className="relative flex-1">
            <button
              ...
              className="flex items-center justify-center gap-2 w-full px-4 py-2 bg-[#183659] text-white rounded-md shadow-sm hover:bg-[#2a4e7a]"
            >
              <ChevronDown className={`w-5 h-5 transition-transform ${showSort ? 'rotate-180' : ''}`} /> Sort By
            </button>
            {showSort && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border z-20 text-sm overflow-hidden">
                <button
                  onClick={() => sortBy("low")}
      ...
      {/* Mobile Filter Section (Dropdown) */}
      {isMobileFilterOpen && (
        <div className="lg:hidden p-4 bg-white rounded-b-lg shadow-lg mb-4">
      ...
      <aside className="hidden lg:block bg-[#183659] pt-5 text-white p-6 rounded-xl mt-40 ml-15 shadow-md w-[280px] h-[530px] flex flex-col gap-1 flex-shrink-0">
      ...
      {/* Main Content Area */}
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-12 lg:ml-10 lg:px-8 lg:pt-10 lg:mt-12">
        {/* Sort by dropdown (Visible on both desktop and mobile, but positioned differently) */}
        <div className="relative mb-4 ml-0 lg:ml-4 hidden lg:block">
          <div
            className="flex items-center gap-2 cursor-pointer"
            onClick={() => setShowSort(!showSort)}
          >
            <h2 className="text-3xl font-semibold text-[#1a1f2c] hidden lg:block">
              BestSelling Products
            </h2>
            <ChevronDown className={`w-6 h-6 text-[#1a1f2c] transition-transform ${showSort ? 'rotate-180' : ''}`} />
          </div>
        ...
        <div className="bg-[#b5c8db] p-4 rounded-2xl relative min-h-[530px] lg:p-8">
          <div
            className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 lg:gap-8 overflow-y-auto"
            style={{
              maxHeight: "530px",
              scrollbarWidth: "thin",
...
//       {/* Reduced height for mobile only */}

//       <div className="w-full h-36 lg:h-45">
...
//       {/* Reduced padding for mobile only */}

//       <div className="bg-white p-2 lg:p-2 text-center">
...
//         {/* Adjusted spacing and font size for mobile only */}

//         <p className="text-gray-900 font-bold text-sm mt-1 lg:hidden">Rs. {price}</p>

//         <p className="text-gray-600 text-xs mt-0.5 lg:hidden">Size: {size}</p>
...
//             className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-8 overflow-y-auto"
